...
class Discrepancy_Model:
    ...
    def predict(self, X):
        self.y_test_pred = pd.Series(self.model.predict(self.X_test), index=self.X_test.index)
        self.y_test_pred = np.clip(self.y_test_pred, 1, 10)

        self.y_train_pred = pd.Series(self.model.predict(self.X_train), index=self.X_train.index)
